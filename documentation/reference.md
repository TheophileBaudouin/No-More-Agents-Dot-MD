...
| `/nma import <name\|keywords> [--yes]` | Search the [community registry](https://theophilebaudouin.github.io/awesome-No-More-Agents-Dot-MD/) by name or keywords (name, tags, category, author) and import a context file into `.pi/context/<name>.md`. An exact name match imports directly; a single keyword match imports directly; multiple matches open a UI picker, or a bullet message (then run `/nma import <name>`) without a UI. When network is disabled (`NMA_NETWORK=0`) the command refuses. The fetched file passes through the same security scan and trust flow as local files (high/critical requires a UI confirmation, or `--yes` without a UI), an existing file asks for overwrite confirmation (or `--yes`), and the import is recorded in the trust store with provenance `registry:<name>` |
| `/nma convert [file] [--yes]` | Split an AGENTS.md-style source (default `AGENTS.md` at the project root; a path argument overrides it) into sections, create `.pi/context/` if needed, print the plan, and hand the agent a conversion brief (sent as a user message — it triggers a turn). The agent writes the atomic rule files itself, following the skill: one topic per file, event/match/action chosen per section, unique kebab names pre-deduped against existing rules, noise lines dropped. The command writes no rule files and never modifies the source; the agent does not overwrite an existing rule file without confirmation (`--yes` pre-approves). Generated files go through the normal security scan at `/nma reload` |
